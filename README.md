...
(These outputs are from the `imp/text-verblessness-opt` branch: de-themed prompts and a shorter sentence cap reduced the earlier cross-topic drift, and a pronoun+verb opener grammar removed the verbless fragments — every sentence now opens with a subject pronoun and a finite verb. Decoding still matches each covertext exactly via prefix matching.)
