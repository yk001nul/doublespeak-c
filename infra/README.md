...
- **Spot worker nodes** — opt-in (`var.worker_use_spot=true`): runs the C2 pool on
  ...
  `node_min_cpu_platform`, so float math is byte-identical). A preemption mid-run is
  recovered by the worker's **SIGTERM re-enqueue** (`worker_app.recover_inflight`) —
  *not* by an automatic Cloud Tasks retry, because the ack-fast worker already 200'd
  the push. Cost: a preempted job restarts from scratch and its replacement node
  cold-reloads the GGUF, and Spot capacity isn't guaranteed. Toggling it recreates
  the node pool (brief worker outage) — do it in a maintenance window. Best once
  traffic can absorb the occasional restart.
