...
- **Modal-without-main-verb openers** ("you will to avoid inaccuracies") — resolved on `imp/modality-fix` (commit `4d64a13`); see "Modality pass" below.
...
- **Subject-verb disagreement** — `OPENING_SUBJECT_GRAMMAR` deliberately does not enforce agreement (any listed verb after any pronoun); the model picks the agreeing form at `temp=0.0` almost always, but occasionally slips ("they arrives a strategic briefing"). Enforcing agreement would need per-pronoun verb sublists; deferred as a rare naturalness nit.
- **Dangling-preposition opener join** ("they have pledged to by promoting") — distinct from the modal fix: a be/have opener whose free tail *ends* on a preposition ("...pledged to") collides with the following continuation phrase's connector ("by ..."), giving a double connector. It's a connector-join issue, not a modality one, so the modality pass left it untouched.
...
### Modality pass (`imp/modality-fix` branch)

Fixes the modal-without-main-verb openers deferred above ("you will to avoid inaccuracies", "they will to meet ..."). `OPENING_SUBJECT_GRAMMAR` (`llm_client.c`) had one flat `verb` list that mixed the modals (`will/would/shall/should/can/could/may/might/must`) and the dummy auxiliary (`do/does/did`) in with the lexical and be/have verbs. Because the opener rule is `phrase ::= subject " " verb (" " word)*`, a *modal alone* could satisfy the verb slot and the free `(" " word)*` tail then slid straight into a connector-like preposition — a modal is grammatically incomplete without a following bare-infinitive main verb.

Fix (commit `4d64a13`): split the verb rule into `verb ::= finite | modal " " baseverb`. `modal` (the modals + `do/does/did`) must now be followed by a `baseverb` (base-form lexical verb): "they will present ...", "they can reduce ...". `finite` keeps the lexical 3ps/base forms, the be/have auxiliaries (`is/are/.../has/have/had`), and the past-tense forms as verbs that can stand alone. **be/have are intentionally left standalone**, not gated like modals: unlike a modal they read fine with the participle the model supplies in the tail at `temp=0.0` ("they have pledged ...", "they are considering ..."), and enumerating -ing/-ed participles to gate them would balloon the list for no observed benefit. `baseverb` is reused both standalone (inside `finite`) and as the modal complement, so the base-form list isn't duplicated. Only `OPENING_SUBJECT_GRAMMAR` changed — the continuation grammar (`build_phrase_grammar`, `connector (" " word)+`) has no modality surface. Compiled-in grammar, identical on both sides, so the encode/decode lockstep holds. Validated `styled_encode` 28/28 at threads=4 on Phi-3.5-mini (2026-07-14): the exact FORMAL_EMAIL repro now encodes as "they will present the findings to avoid ...".

### Sample style-mode outputs (verified 2026-07-14, commit `4d64a13`, branch `imp/modality-fix`, threads=4)
...
| `FORMAL_EMAIL` | The team will present the quarterly results to stakeholders on Friday. | `hi` | they will present the findings to avoid premature conclusions. they present the fiscal summary to the board by leveraging data insights. they arrives a strategic briefing for the investors to join fiscal review team. they have enhanced the methodology to meet shareholders. |

These samples reflect the coherence pass (de-themed prompts + `CLAUSE_END_MAX_PHRASES=2`), the verblessness pass (pronoun + finite-verb openers), the style-register pass (distinct registers), and the modality pass (modals now take a main verb). The four styles differ visibly in register — chat is plain ("takes his ride", "shares his new route"), email is corporate-formal ("leveraging data insights"), blog is first-person narrative ("i was exploring", "my hiking buddy"), news is institutional ("they have pledged", "transnational ecological organizations"). The FORMAL_EMAIL opener is now well-formed ("they will present the findings ...", was "you will to avoid inaccuracies"). Only INFORMAL_CHAT changed between the style-register run and this one; the other three topics' modal-free openers were already stable, and NEWS_ARTICLE is byte-identical because its openers use be/have (`are`/`have`), not modals, which the fix deliberately leaves untouched. Remaining visible nits — pronoun-referent drift across sentences ("i was exploring ... she starts ... he starts"), a subject-verb slip ("they arrives"), and the dangling-preposition join ("they have pledged to by promoting") — are the deferred items above.
...
## No LLM fallbacks — a failed `/completion` must abort the job

**Do not reintroduce a local fallback distribution.** `llm_client.c` used to answer a failed `/completion` (unreachable, timeout, unparseable/empty JSON) with a uniform distribution over a hardcoded 20-entry table — `FALLBACK_PHRASES`, `FALLBACK_WORDS`, `FALLBACK_SYLLABLES` — plus a fixed `DIGRESSION_FALLBACK_ANSWER` string. The intent was that a server hiccup shouldn't lose a multi-minute run, and because both sides substituted the same table deterministically the PRNG stayed in lockstep and the round-trip still succeeded.

That is exactly what made it dangerous: **a total LLM outage presented as success.** Found live 2026-07-22 on `meteor-stego-1`, where every call was timing out and the covertext came back as "and the world from the start. in the morning with the team. by the river from the start. on the street on the street." — every phrase from the table. Covertext drawn from a 20-phrase vocabulary is trivially machine-detectable, so the library silently stopped providing the one property it exists for, while reporting `status: done`.

All entry points now return NULL on any failure, which callers already map to `METEOR_ERR_LLM`. Losing a long run to a transient error is strictly better than emitting detectable covertext.

**The existing suite cannot catch this class of bug.** `roundtrip`, `determinism` and `styled_encode` only assert `decode(encode(m)) == m`, which a shared fallback satisfies perfectly — all of them passed at 28/28 throughout the outage. **Treat "the tests pass" as no evidence the LLM was consulted at all.** `tests/test_llm_failure.c` is the guard: it needs no llama-server and no model, and asserts encode and decode both fail with `METEOR_ERR_LLM` against a dead port in both style and syllable mode.

Field signature of a degraded run, for diagnosing from live data: every step yields exactly `beta` bits (a uniform 2^beta-candidate distribution fills every slot), so the run takes exactly `ceil(total_bits/beta)` steps, and wall-clock per step pins to `llm_timeout_ms`.
