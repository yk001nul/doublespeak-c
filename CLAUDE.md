...
Critical server flags for determinism: `--threads 1 --temp 0.0 --seed 42 --no-mmap`. The start scripts also pass `--parallel 1 --no-cont-batching --slots` (single decode slot, no cross-request batching, and the `/slots` KV-cache-erase endpoint the library uses in prompt-cache mode). See `ARCHITECTURE.md §6` for the full determinism checklist.

## Performance tuning (`imp/perf-opt-2` branch)

Essentially all encode/decode wall-clock time is LLM inference — one or more blocking HTTP round-trips to `llama-server` per Meteor step, 8–20+ steps per short message. Three levers, in decreasing safety:
...
2. **Thread count — `METEOR_NUM_THREADS` env var (default 1).** The start scripts read it and pass `--threads N`. llama.cpp's CPU backend is reproducible for a *fixed* thread count, so raising this is a near-linear speedup — but the count is now part of the shared protocol: **encoder and decoder must use the identical value**, and cross-machine use is only safe when both machines run the same count. Default stays 1 so existing behavior is unchanged. Validate any new value with `ctest -R determinism` before trusting it.

3. **Prompt caching — `METEOR_PROMPT_CACHE` env var (default off).** When enabled (`1`/`true`/`yes`/`on`), every `/completion` request sets `"cache_prompt": true` and pins `"id_slot": 0`, and `meteor_encode`/`meteor_decode` call `llm_client_erase_slot()` at the start of each run. Because the covertext (the O(n)-growing part of the prompt) is a strict prefix-extension each step, this turns per-run prefill from ≈O(steps²) into ≈O(steps). Determinism is preserved **only** because both sides start from an identical erased slot and, with `--parallel 1`, evolve byte-identical cache boundaries — but this property must be **confirmed on the target model** (run `ctest -R "determinism|roundtrip|styled_encode"` with `METEOR_PROMPT_CACHE=1`, ideally also after warming the server with unrelated requests) before relying on it. It is off by default for exactly this reason: when off, request bodies are byte-identical to the pre-change code (`cache_prompt: false`, no `id_slot`), so determinism is unchanged. Requires the server started with `--slots` (the start scripts add it).
