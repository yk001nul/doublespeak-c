...
|---|---|---|---|
| `INFORMAL_CHAT` | John goes to the office using his car every morning. | `hi` | he heads to the office to avoid meeting tight deadlines. his ride to the office using his personal vehicle. his car to the office with the department head. that takes public transport with the design team. he commutes to avoid missing his morning meeting. his ride to work to avoid car trouble. that to office. |
| `NEWS_ARTICLE` | The government announced new policies to reduce carbon emissions by 2030. | `hi` | it introduces to avoid reliance on fossil fuels. this entity establishes by implementing stricter regulations. they set forth to meet sustainable development groups. they commence to meet international environmental agencies. the government introduces to avoid environmental degradation. the administration adopts to avoid regulatory penalties for noncompliance with environmen. |
| `CASUAL_BLOG` | Sarah spent the whole weekend hiking in the mountains with her dog. | `hi` | he spent while avoiding rocky terrain. her pooch and me by trail maps. her dog and me to join a mountain club. it underwent a slight alteration to meet fellow hikers. his weekend escapade to avoid getting lost. he and his pup explored. |
| `FORMAL_EMAIL` | The team will present the quarterly results to stakeholders on Friday. | `hi` | the team will present to avoid unnecessary delays. it updates using online tools. she delivers with stakeholders. the process has seen with key partners. they report to avoid data discrepancies. they communicate results to avoid inaccuracies. the team updates using online tools. she coordinates the presentation of financial summaries. |

These samples reflect the de-themed axis prompts and the `CLAUSE_END_MAX_PHRASES=2` sentence cap on `imp/text-coherence-opt`: vocabulary tracks each topic (contrast the earlier office/commute drift) and each sentence is now an opener plus at most one modifier, so the earlier stacked-modifier run-ons are gone. The shorter form surfaces a pre-existing verbless/fragment-opener issue (e.g. "that to office.") that is the next item being iterated on; one sample also truncates mid-word ("environmen.") because short messages now span more sentences.
